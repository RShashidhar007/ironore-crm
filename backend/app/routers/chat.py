...
def _sanitize_reply(reply: str, current_customer_cid: Optional[str], db: Session) -> str:
    ...
    if re.search(r"(?is)\bselect\s+.*\bfrom\b", reply):
    ...
    try:
        ...
        # If database session is in a bad state (e.g., after a failed transaction),
        # just skip the CID check and return the reply sanitized of obvious patterns
        print(f"Warning: Could not perform CID sanitization check: {e}")
    return reply
            ...
            if complaint:
                ...
                if complaint.CreatedBy != current_user.User_Id:
                    ...
                    if complaint.Status == "Resolved":
                        ...
                        # Solution should be generated by the background scheduler
                        # If solution exists, show it to the customer
                        
                        if complaint.Solution and complaint.Solution.strip():
                            # Solution has been generated - show it to customer
                            overall_status = "Resolved"
                            verified_data = f"Complaint {complaint_id} resolved with solution"
                            
                            # Format solution with timestamps
                            template_reply = (
                                f"Hello {current_user.User_Name},\n\n"
                                f"**Complaint ID:** {complaint.ComplaintID}\n"
                                f"**Status:** ✅ Resolved\n\n"
                            ...
                            # Solution is being generated by background scheduler
                            overall_status = "In Progress"
                            verified_data = f"Complaint {complaint_id} in progress - solution being generated"
                            
                            template_reply = (
                                f"Hello {current_user.User_Name},\n\n"
                                f"**Complaint ID:** {complaint.ComplaintID}\n"
                                f"**Status:** ⏳ Solution is being generated\n\n"
                                f"All reviews have been completed. Our system is now generating your resolution.\n"
                                f"This will be ready within the next few minutes. Please check back shortly!"
                            )
                    else:
